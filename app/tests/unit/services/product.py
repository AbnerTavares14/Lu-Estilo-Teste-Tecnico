...
from app.models.domain.product import ProductModel # Para type hinting e mock spec
...
def mock_product_repo(mocker):
    """Fixture para criar um mock do ProductRepository."""
    return mocker.Mock()

@pytest.fixture
def product_service(mock_product_repo):
    """Fixture para criar uma instância do ProductService com o repositório mockado."""
...
def sample_product_schema() -> ProductSchema:
    return ProductSchema(
        ...
        image_url=VALID_IMAGE_URL_FOR_TEST
    )

@pytest.fixture
def sample_product_model(mocker, sample_product_schema: ProductSchema) -> Mock: # Retorna um Mock configurado
    # Para testes unitários do serviço, um Mock com spec é geralmente suficiente
    # e evita a necessidade de instanciar o modelo SQLAlchemy real.
    ...
    mock_model.image_url = sample_product_schema.image_url
    return mock_model
...
class TestProductService:
    ...
    def test_update_product_barcode_changed_to_conflicting(
        ...
        update_data_conflicting_barcode = ProductSchema(
            ...
            section="Sect", stock=10, image_url=VALID_IMAGE_URL_FOR_TEST
        )
    ...
    def test_update_product_db_integrity_error_on_update(
        ...
        product_id = sample_product_model.id
        # Barcode não mudou ou mudou para um que passou na checagem do serviço
        update_data = sample_product_schema

        mock_product_repo.get_product_by_id.return_value = sample_product_model
        # Se o barcode mudou, get_product_by_barcode retornaria None
        ...
        assert exc_info.value.status_code == status.HTTP_409_CONFLICT
        assert exc_info.value.detail == "Update failed due to barcode conflict (DB integrity)"
    ...
    def test_update_product_stock_decrease_success(
        ...
        for attr in ['id', 'description', 'price', 'barcode', 'section', 'expiry_date', 'image_url']:
